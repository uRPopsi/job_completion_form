<div class="p-6 lg:p-8 bg-white border-b border-gray-200">
    <form wire:submit.prevent="generatePDF">
        ...
        <div>
            ...
            <x-input id="client" class="block mt-1 w-full" type="text" wire:model="client" required autofocus />
        </div>

        <div class="mt-4">
            <x-label for="project" value="Project" />
            <x-input id="project" class="block mt-1 w-full" type="text" wire:model="project" required />
        </div>

        <div class="mt-4">
            <x-label for="po" value="PO" />
            <x-input id="po" class="block mt-1 w-full" type="text" wire:model="po" required />
        </div>

        <div class="mt-4">
            <x-label for="status" value="Status" />
            <x-input id="status" class="block mt-1 w-full" type="text" wire:model="status" required />
        </div>

        <div class="mt-4">
            <x-label for="completion_date" value="Completion Date" />
            <x-input id="completion_date" class="block mt-1 w-full" type="date" wire:model="completion_date" required />
        </div>

        <div class="mt-4">
            <x-label for="invoice" value="Invoice" />
            <x-input id="invoice" class="block mt-1 w-full" type="text" wire:model="invoice" required />
        </div>
        ...
        <button type="button" class="mt-2 bg-blue-500 text-white px-3 py-2" wire:click="addItem">+ Add Row</button>

        <div class="flex flex-col items-center w-full mt-4">
            <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded">Generate PDF</button>
        </div>
    </form>
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            Livewire.on('pdfGenerated', (pdfUrl) => {
                window.location.href = pdfUrl; // Redirect to download the file
            });
        });
    </script>
</div>
